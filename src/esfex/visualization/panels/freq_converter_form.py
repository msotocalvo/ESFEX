"""Frequency converter properties form."""
...
class FreqConverterForm(QWidget):
    """Property editor for a frequency converter."""
    ...
    def __init__(self, model: GuiModel, parent=None):
        ...
        # ── Identity ──
        id_group = QGroupBox(tr("freq_form.group_identity"))
        id_layout = QFormLayout(id_group)

        self._system_label = QLabel("")
        id_layout.addRow(tr("freq_form.system"), self._system_label)

        self._name = QLineEdit()
        self._name.editingFinished.connect(self._on_changed)
        id_layout.addRow(tr("freq_form.name"), self._name)

        outer.addWidget(id_group)

        # ── Topology ──
        topo_group = QGroupBox(tr("freq_form.group_topology"))
        topo_layout = QFormLayout(topo_group)

        # Connection labels (read-only, auto-detected from lines)
        self._from_connections_label = QLabel(tr("freq_form.no_connections"))
        self._from_connections_label.setWordWrap(True)
        topo_layout.addRow(tr("freq_form.connected_from"), self._from_connections_label)

        self._to_connections_label = QLabel(tr("freq_form.no_connections"))
        self._to_connections_label.setWordWrap(True)
        topo_layout.addRow(tr("freq_form.connected_to"), self._to_connections_label)

        # Editable bus selectors (see transformer_form.py for context).
        self._from_bus_combo = QComboBox()
        self._from_bus_combo.currentIndexChanged.connect(
            lambda _: self._on_bus_combo_changed("from")
        )
        topo_layout.addRow(tr("freq_form.from_bus"), self._from_bus_combo)
        self._to_bus_combo = QComboBox()
        self._to_bus_combo.currentIndexChanged.connect(
            lambda _: self._on_bus_combo_changed("to")
        )
        topo_layout.addRow(tr("freq_form.to_bus"), self._to_bus_combo)

        info_label = QLabel(tr("freq_form.draw_lines_hint"))
        info_label.setWordWrap(True)
        info_label.setObjectName("infoLabel")
        topo_layout.addRow(info_label)

        self._from_freq = QDoubleSpinBox()
        self._from_freq.setRange(0.1, 400)
        self._from_freq.setDecimals(1)
        self._from_freq.editingFinished.connect(self._on_changed)
        self._from_freq.setEnabled(False)
        self._from_freq.setToolTip(tr("electrical.inherited_tooltip"))
        topo_layout.addRow(tr("freq_form.from_freq"), self._from_freq)

        self._to_freq = QDoubleSpinBox()
        self._to_freq.setRange(0.1, 400)
        self._to_freq.setDecimals(1)
        self._to_freq.editingFinished.connect(self._on_changed)
        self._to_freq.setEnabled(False)
        self._to_freq.setToolTip(tr("electrical.inherited_tooltip"))
        topo_layout.addRow(tr("freq_form.to_freq"), self._to_freq)

        outer.addWidget(topo_group)

        # ── Capacity ──
        cap_group = QGroupBox(tr("freq_form.group_capacity"))
        ...
        cap_layout.addRow(tr("freq_form.rated_power"), self._rated_power)

        self._min_power = QDoubleSpinBox()
        self._min_power.setRange(0, 1e6)
        self._min_power.editingFinished.connect(self._on_changed)
        cap_layout.addRow(tr("freq_form.min_power"), self._min_power)

        outer.addWidget(cap_group)

        # ── Efficiency ──
        eff_group = QGroupBox(tr("freq_form.group_efficiency"))
        eff_layout = QFormLayout(eff_group)

        self._eff_a_to_b = QDoubleSpinBox()
        self._eff_a_to_b.setRange(0, 1)
        self._eff_a_to_b.setDecimals(4)
        self._eff_a_to_b.setSingleStep(0.01)
        self._eff_a_to_b.editingFinished.connect(self._on_changed)
        eff_layout.addRow(tr("freq_form.eff_a_to_b"), self._eff_a_to_b)

        self._eff_b_to_a = QDoubleSpinBox()
        self._eff_b_to_a.setRange(0, 1)
        self._eff_b_to_a.setDecimals(4)
        self._eff_b_to_a.setSingleStep(0.01)
        self._eff_b_to_a.editingFinished.connect(self._on_changed)
        eff_layout.addRow(tr("freq_form.eff_b_to_a"), self._eff_b_to_a)
        ...
        eff_layout.addRow(tr("freq_form.standby_losses"), self._standby_losses)

        outer.addWidget(eff_group)

        # ── Reactive Power ──
        q_group = QGroupBox(tr("freq_form.group_reactive"))
        ...
        q_layout.addRow(tr("freq_form.q_min"), self._q_min)

        self._q_max = QDoubleSpinBox()
        self._q_max.setRange(0, 1e6)
        self._q_max.editingFinished.connect(self._on_changed)
        q_layout.addRow(tr("freq_form.q_max"), self._q_max)

        outer.addWidget(q_group)

        # ── Impedance ──
        z_group = QGroupBox(tr("freq_form.group_impedance"))
        ...
        z_layout.addRow(tr("freq_form.impedance"), self._impedance)
        ...
        z_layout.addRow(tr("freq_form.resistance"), self._resistance)

        outer.addWidget(z_group)

        # ── Economics ──
        econ_group = QGroupBox(tr("freq_form.group_economics"))
        ...
        econ_layout.addRow(tr("freq_form.fixed_cost"), self._fixed_cost)
        ...
        econ_layout.addRow(tr("freq_form.variable_cost"), self._variable_cost)

        outer.addWidget(econ_group)

        # ── Lifecycle ──
        life_group = QGroupBox(tr("freq_form.group_lifecycle"))
        ...
        life_layout.addRow(tr("freq_form.lifetime"), self._lifetime)

        self._initial_age = QSpinBox()
        self._initial_age.setRange(0, 100)
        self._initial_age.editingFinished.connect(self._on_changed)
        life_layout.addRow(tr("freq_form.initial_age"), self._initial_age)
        ...
        life_layout.addRow(tr("freq_form.degradation"), self._degradation)
    ...
    def load_element(self, element_id: str):
        ...
        try:
            ...
            return
        if idx >= len(self._model.state.freq_converters):
            return
        self._multi_ids = None
        self._current_idx = idx
        conv = self._model.state.freq_converters[idx]

        self._system_label.setText(tr("freq_form.system_label", name=self._model.state.name))
        self._updating = True

        # Propagate frequency from connected buses
        self._model.propagate_bus_to_element("freq_converter", raw_id)

        self._name.setText(conv.name)

        # Query and display connections
        connections = self._model.get_connected_elements("freq_converter", raw_id)
        ...
        if connections["from"]:
            ...
            self._from_connections_label.setText(tr("freq_form.draw_lines_hint"))
            self._from_connections_label.setStyleSheet(
        ...
        if connections["to"]:
            ...
            self._to_connections_label.setText(tr("freq_form.draw_lines_hint"))
            self._to_connections_label.setStyleSheet(f"color: {c.text_disabled}; padding: 4px;")

        self._from_freq.setValue(conv.from_frequency_hz)
        self._to_freq.setValue(conv.to_frequency_hz)
        self._rated_power.setValue(conv.rated_power_mva)
        self._min_power.setValue(conv.min_power_mva)
        self._eff_a_to_b.setValue(conv.efficiency_a_to_b)
        self._eff_b_to_a.setValue(conv.efficiency_b_to_a)
        ...
        from esfex.visualization.data.default_colors import FREQ_CONVERTER
        self._style_widget.set_default_color(FREQ_CONVERTER)
    ...
    def _populate_bus_combo(self, combo: QComboBox, current_bus_id: str):
        ...
        for i, (bid, bus) in enumerate(self._model.state.buses.items()):
            label = f"{bid}  ({bus.voltage_kv:g} kV, {bus.frequency_hz:g} Hz)"
    ...
    def _on_bus_combo_changed(self, side: str):
        if self._updating or self._current_idx is None:
            return
        if self._current_idx >= len(self._model.state.freq_converters):
            return
        conv = self._model.state.freq_converters[self._current_idx]
        ...
        if side == "from":
            ...
            conv.from_frequency_hz = bus.frequency_hz or conv.from_frequency_hz
            self._updating = True
            self._from_freq.setValue(conv.from_frequency_hz)
            self._updating = False
        else:
            ...
            conv.to_frequency_hz = bus.frequency_hz or conv.to_frequency_hz
            self._updating = True
            self._to_freq.setValue(conv.to_frequency_hz)
            self._updating = False
    ...
    def _field_map(self):
        return [
            ...
            ("efficiency_a_to_b", self._eff_a_to_b),
            ("efficiency_b_to_a", self._eff_b_to_a),
    ...
    def load_elements(self, element_ids: list[str]):
        ...
        convs = self._model.state.freq_converters
        ...
        self._system_label.setText(tr("freq_form.system_label", name=f"{len(element_ids)} freq. converters selected"))
        self._updating = True
    ...
    def _on_changed(self):
        ...
        if self._multi_ids:
            ...
            instances = [self._model.state.freq_converters[self._current_idx]]
        else:
            return
